...
class MarketServiceProvider extends ServiceProvider
{
    ...
    {
        $viewPath = resource_path('views/modules/market');
        ...
        $this->loadViewsFrom(array_merge(array_map(function ($path) {
            return $path . '/modules/market';
        }, \Config::get('view.paths')), [$sourcePath]), 'market');
    }
    ...
    {
        $langPath = resource_path('lang/modules/market');

        if (is_dir($langPath)) {
            $this->loadTranslationsFrom($langPath, 'market');
        } else {
